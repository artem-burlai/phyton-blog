...
def index():
    ...
    for post in reversed(result):
         posts.append({"id": post[0], "title": post[1], "content": post[2],
                       "author_id": post[3], "username": post[4], "like": post[5]})
...
def add_post():
    if request.method == "POST":
        ...
        cursor.execute(
            'INSERT INTO post (title, content, author_id, username) VALUES(?, ?, ?, ?)',
            (title, content, current_user.id, current_user.username)
        )
        connection.commit()
...
def post(post_id):
    ...
    print(result)
    post_dict = {"id": result[0], "title": result[1], "content": result[2], "username": result[4]}
